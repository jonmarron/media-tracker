...
> _"Read BACKLOG.md and work on ticket X"_
>
> Or copy-paste a single ticket as your prompt. Mark tickets as `[x]` when done.
> Work through them in order — later tickets depend on earlier ones.
...
1. **Work ticket by ticket.** Don't ask Claude Code to build the whole app at once. Smaller, focused prompts produce better results.

2. **Reference this file.** Say: _"Read BACKLOG.md. I want to work on Ticket 3. The previous tickets are already done."_

3. **Review before moving on.** After each ticket, run the app, check the result, and ask Claude Code to fix anything before starting the next one.

4. **Add context when needed.** If you have design preferences (dark mode, specific colors, a font you like), mention them when working on Ticket 2 — that's where the visual identity gets established.

5. **Use the commit cycle.** After each completed ticket, commit with a message like `feat: ticket-2 app layout shell`. This gives you clean rollback points.
